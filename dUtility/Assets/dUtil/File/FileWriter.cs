...
namespace du.File {
    ...
    public class FWriter : IFWriter {
        ...
        #region ctor/dtor
        private FWriter(string filePath) { m_filePath = filePath; }
        #endregion
        ...
        public static IFWriter FromMyData4Rewrite(string filePath) {
            var writer = new FWriter(du.App.AppManager.DataPath + filePath);
            writer.OpenRewrite();
            return writer;
        }
        /// <param name="filePath"> du.App.AppManager.DataPath以下、拡張子を含めてファイル名を指定 </param>
        public static IFWriter FromMyData4Append(string filePath) {
            var writer = new FWriter(du.App.AppManager.DataPath + filePath);
            writer.OpenAppend();
            return writer;
        }
